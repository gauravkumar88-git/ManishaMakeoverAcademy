...
export default function StudentClasses() {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      setPurchasedClasses(
        purchaseRes.data.purchases.map((item) => item.classId.toString()),
      );
                    ...
                    <div className="absolute top-3 left-3 flex gap-2">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium border capitalize ${
                          statusColors[cls.status] || statusColors.upcoming
                        }`}
                      >
                        {cls.status}
                      </span>

                      <span className="px-2 py-1 rounded-full text-xs font-medium border border-white/20 text-white/70 capitalize">
                        {cls.category}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      {cls.status === "live" ? (
                        hasAccess ? (
                          ...
                          <button
                            disabled
                            className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-medium text-gray-400 bg-white/5"
                          >
                            <FiLock size={15} />
                            {cls.accessType === "individual"
                              ? "Purchase Required"
