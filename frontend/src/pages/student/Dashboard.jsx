...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [clsRes, purchaseRes, notesRes, attRes, notifRes] =
          await Promise.all([
            api.get("/classes?status=upcoming&limit=3"),
            api.get("/class-purchase/my").catch(() => ({
              data: { purchases: [] },
            })),
            api.get("/notes?limit=3").catch(() => ({
              data: { notes: [] },
            })),
            api.get("/attendance/my").catch(() => ({
              data: { attended: 0, totalClasses: 0, percentage: 0 },
            })),
        ...
        setPurchasedClasses(
          purchaseRes.data.purchases.map((p) => p.classId.toString()),
        );
      } catch (e) {
        /* silent */
      } finally {
  ...
  const isSubscribed =
    user?.subscriptionActive && new Date(user?.subscriptionExpiry) > new Date();
  const planColors = {
    none: "#6b7280",
    basic: "#E91E8C",
    premium: "#9C27B0",
    ...
    <DashboardLayout>
      <div className="space-y-8">
        ...
        <motion.div
          ...
          style={{
            background:
              "linear-gradient(135deg, #1a0020 0%, #3d0030 50%, #1a0020 100%)",
            border: "1px solid rgba(233,30,140,0.2)",
          }}
        >
          <div
            className="orb w-64 h-64 bg-pink-600 -right-20 -top-20"
            style={{ opacity: 0.2 }}
          />
          <div
            className="orb w-48 h-48 bg-purple-600 -left-10 -bottom-10"
            style={{ opacity: 0.15 }}
          />
          <div className="relative">
            <div className="flex items-center gap-4 mb-4">
              <img
                src={
                  user?.avatar ||
                  `https://ui-avatars.com/api/?name=${user?.name}&background=E91E8C&color=fff&size=80`
                }
                alt=""
                className="w-16 h-16 rounded-2xl object-cover ring-2 ring-pink-500/40"
              />
              <div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {isSubscribed && (
                <span className="inline-flex items-center gap-1 px-3 py-2 rounded-full text-xs text-green-400 bg-green-500/10 border border-green-500/20">
                  <FiCheckCircle size={12} /> Active till{" "}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            color="#10B981"
            sub={`${data.attendance?.attended || 0} classes attended`}
          />
          <StatCard
            icon={FiAward}
            label="Certificates"
            value="—"
            color="#F59E0B"
            sub="Earn by completing courses"
          />
        </div>
        ...
        {!isSubscribed && (
          <motion.div
            ...
            style={{
              background:
                "linear-gradient(135deg, rgba(233,30,140,0.12) 0%, rgba(123,45,139,0.12) 100%)",
          ...
          >
            <div>
              ...
              <p className="text-gray-400 text-sm">
                Subscribe to join live classes, download notes & earn
                certificates.
              </p>
            </div>
            <Link
                  ...
                  const hasAccess =
                    cls.accessType === "individual"
                      ...
                      : user?.subscriptionActive &&
                        ...
                        ["basic", "premium", "vip"].indexOf(
                          user.subscriptionPlan,
                        ) >=
                          ["basic", "premium", "vip"].indexOf(cls.requiredPlan);
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-gray-400 text-xs mt-0.5 capitalize">
                        {note.type} • {note.classId?.title || "Class"}
                      </p>
                    </div>
                    {isSubscribed && (
                      <a
          ...
          <div className="glass rounded-2xl p-6">
            ...
            <div className="space-y-2">
              {data.notifications.slice(0, 4).map((n) => (
                <div
                  key={n._id}
                  className={`flex items-start gap-3 p-3 rounded-xl transition-colors ${!n.read ? "bg-pink-500/5 border border-pink-500/10" : "hover:bg-white/5"}`}
                >
                  <div
                    className={`w-2 h-2 rounded-full mt-2 flex-shrink-0 ${n.read ? "bg-gray-600" : "bg-pink-500"}`}
                  />
                  <div>
