...
export default function StudentNotes() {
  ...
  const isSubscribed =
    user?.subscriptionActive && user?.subscriptionPlan !== "none";
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [notesRes, purchaseRes] = await Promise.all([
          api.get("/notes"),
...
console.log("NOTES:", notesRes.data.notes);
      } catch (err) {
        console.log(err);
      } finally {
        setLoading(false);
  ...
  const hasAccess = (note) => {
    // Subscription class
    if (note.classId?.accessType !== "individual") {
      return isSubscribed;
    }

    // Individual class
    return purchasedClasses.includes(note.classId?._id);
  };

  const filtered = notes.filter((n) => {
    const matchSearch =
      n.title.toLowerCase().includes(search.toLowerCase()) ||
      n.classId?.title?.toLowerCase().includes(search.toLowerCase());
          ...
          <div className="glass rounded-2xl p-5 border border-pink-500/20 flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-gray-400 text-sm mt-1">
                Subscribe to download all study materials and PDFs.
              </p>
            </div>
            <a
              href="/pricing"
            ...
            {filtered.map((note, i) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium border capitalize ${TYPE_COLORS[note.type] || TYPE_COLORS.other}`}
                  >
                    {note.type}
                  </span>
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  >
                    <FiDownload size={13} />{" "}
                    {hasAccess(note) ? "Download" : "Locked"}
                  </button>
                  {hasAccess(note) && (
                    <a
                      href={note.pdfUrl}
