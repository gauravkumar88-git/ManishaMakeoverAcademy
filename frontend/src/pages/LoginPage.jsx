...
export default function LoginPage() {
  ...
  //   // Google Sign-In (requires Google Identity Services script in index.html)
  ...
  return (
    <div className="min-h-screen flex" style={{ background: '#0D0010' }}>
      {/* Left – Visual */}
      <div className="hidden lg:flex flex-1 items-center justify-center relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #1a0010 0%, #3d0030 100%)' }}>
        ...
        <div className="relative text-center px-12">
          ...
          <div className="mt-10 space-y-4">
            {['🎥 Live Interactive Classes', '📄 Premium PDF Notes', '🏆 Industry Certificates', '👩‍🏫 Expert Mentorship'].map(f => (
              <div key={f} className="flex items-center gap-3 text-pink-100 text-sm">
                <span>{f}</span>
              </div>
            ))}
          </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} className="w-full max-w-md">
          <div className="lg:hidden text-center mb-8">
            ...
            <h1 className="font-display text-2xl font-bold text-white">Manisha Makeover Academy</h1>
          </div>

          <h2 className="font-display text-3xl font-bold text-white mb-2">Welcome back 💄</h2>
          <p className="text-gray-400 mb-8">Sign in to continue your beauty journey</p>
...
/>
          {/* <button onClick={handleGoogle} className="w-full flex items-center justify-center gap-3 py-3 rounded-xl glass border border-white/10 text-white text-sm font-medium hover:bg-white/5 transition-all mb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="relative">
              ...
              <input type="email" placeholder="Email address" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })}
                className="input-beauty pl-11" required />
            </div>
            <div className="relative">
              <FiLock className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
              <input type={showPwd ? 'text' : 'password'} placeholder="Password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })}
                className="input-beauty pl-11 pr-11" required />
            ...
            <div className="text-right">
              <Link to="/forgot-password" className="text-pink-400 text-sm hover:text-pink-300 transition-colors">Forgot password?</Link>
            </div>
          ...
          <p className="text-center text-gray-400 text-sm mt-6">
            ...
            <Link to="/register" className="text-pink-400 hover:text-pink-300 font-medium transition-colors">Create one</Link>
          </p>
        </motion.div>
      </div>
    </div>
